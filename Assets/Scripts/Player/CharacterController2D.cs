...
namespace Prime31
{
	...
	public class CharacterController2D : MonoBehaviour
	{
		...
		public class CharacterCollisionState2D
		{
			...
			public void reset()
			{
				grounded = right = left = above = below = becameGroundedThisFrame = movingDownSlope = false;
				slopeAngle = 0f;
			}
		...
		[Range(0, 1)]
		public float onGroundTolerance = 0;
		...
		public bool isGrounded { get { return collisionState.below || collisionState.grounded; } }
		...
		void moveVertically(ref Vector3 deltaMovement)
		{
			...
			var paddedRayDistance = isGoingUp ? rayDistance : rayDistance + onGroundTolerance;
			...
			for (var i = 0; i < totalVerticalRays; i++)
			{
				...
				DrawRay(ray, rayDirection * paddedRayDistance, Color.red);
				_raycastHit = Physics2D.Raycast(ray, rayDirection, paddedRayDistance, mask);
				if (_raycastHit)
				{
					// set our new deltaMovement and recalculate the rayDistance taking it into account
					paddedRayDistance = Mathf.Abs(_raycastHit.point.y - ray.y);

					//Real collision
					if (rayDistance >= paddedRayDistance)
					{
						...
						// we add a small fudge factor for the float operations here. if our rayDistance is smaller
						// than the width + fudge bail out because we have a direct impact
						if (paddedRayDistance < _skinWidth + kSkinWidthFloatFudgeFactor)
							break;
					...
					{
						//Just set the OnGround flag to true
